        ...
        if ($request->isMethod('post')) { // WHETHER 'Add a Product' or 'Update a Product' <form> is submitted (THE SAME <form>)!!
            ...
            if ($request->hasFile('product_image')) {
                ...
                if ($image_tmp->isValid()) {
                    ...
                    // ✅ Large Image - Keep quality, maintain ratio
                    Image::make($image_tmp)->resize(1000, null, function ($constraint) {
                        $constraint->aspectRatio(); // maintain ratio
                        $constraint->upsize();      // prevent blur if small image
                    })->save($largeImagePath);
                    ...
                    // ✅ Small Thumbnail Image — square crop for grid
                    Image::make($image_tmp)->fit(250, 250)->save($smallImagePath);
